...
func _ready() -> void:
	add_to_group("player")
	# cuddlebug 幸存者（美术翻修）：四方向 idle/walk + 持枪 shoot + 近战 stab（武器握在手里）
	body.sprite_frames = Fx.cb_frames([
		["characters/cb_player_idle.png", "idle", 3, 6.0, true],
		["characters/cb_player_walk.png", "walk", 5, 11.0, true],
		["characters/cb_player_shoot.png", "shoot", 5, 20.0, false],
		["characters/cb_player_stab.png", "stab", 5, 20.0, false],
	])
	body.offset = Vector2(0, -14)  # 脚底贴近碰撞中心（俯视 3/4 视角）
	body.play("idle_down")


## 武器侧调用：朝攻击方向播动作。action="shoot"（远程持枪）或 "stab"（近战）。
func play_attack(direction: Vector2, action: String = "stab") -> void:
	if _dead:
		return
	_facing = Fx.dir_name(direction)
	_attack_timer = 0.28
	body.speed_scale = 1.0
	body.play(action + "_" + _facing)
